...
export function Step0Setup({ docDates, setDocDates, emailConfig, onNext, onEditEmail }: Props) {
  ...
  return (
    <>
        ...
        <VisaPanel>
          ...
          <VisaPanelBody tight>
            <DocumentList>
              {ACTIVE_DOCS.map((d) => {
                const v = docDates[d.id] || {};
                const fromInputId = `${d.id}-from`;
                const toInputId = `${d.id}-to`;
                const singleInputId = `${d.id}-single`;
                      ...
                      <div className="mt-2 flex flex-col gap-2">
                        {d.dateFormat === "range" ? (
                          <div className="flex flex-wrap items-center gap-2 font-mono text-[11px]">
                            <label
                              className="w-9 shrink-0 tracking-[0.05em] text-(--ink-3) uppercase"
                              htmlFor={fromInputId}
                            >
                              FROM
                            </label>
                            <input
                              id={fromInputId}
                              className="rounded-[4px] border border-(--rule-2) bg-(--paper) px-2 py-1 text-xs text-(--ink) transition-[border-color,box-shadow] outline-none focus:border-(--accent) focus:shadow-[0_0_0_2px_var(--accent-soft)]"
                              onChange={(e) => updateDate(d.id, "from", e.target.value)}
                              type="date"
                              value={v.from || ""}
                            />
                            <label
                              className="ml-2 tracking-[0.05em] text-(--ink-3) uppercase"
                              htmlFor={toInputId}
                            >
                              TO
                            </label>
      ...
      <details className="mt-3">
        <summary className="cursor-pointer py-2 font-mono text-[11px] text-(--ink-3)">
      ...
      <VisaButtonRow align="between">
        <VisaButton size="sm" variant="ghost">
          ⟳ Re-seed from <VisaMonoText className="ml-1">Document list</VisaMonoText>
        </VisaButton>
