...
import { type VisaSessionRecord, type WorkflowDocumentState } from "@/lib/visa-workflow";
...
export function SummaryTile({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[1.5rem] border border-border/70 bg-card/80 px-4 py-4">
      <p className="text-xs tracking-[0.22em] text-muted-foreground uppercase">{label}</p>
  ...
  return (
    <section className={cn("rounded-[1.75rem] border border-border/70 bg-card/90 p-5", className)}>
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="text-xs tracking-[0.24em] text-muted-foreground uppercase">Live log</span>
      </div>
