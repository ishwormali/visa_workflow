      ...
      <div className="space-y-6">
        ...
        <div className="rounded-[1.75rem] border border-border/70 bg-card/80 p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs tracking-[0.24em] text-muted-foreground uppercase">Re-seed</p>
          ...
          {seedReview.length ? (
            <div className="mt-4 space-y-3">
              {seedReview.map((docType) => {
                const checkboxId = `seed-review-doc-type-${docType.id}`;

                return (
                  <div
                    key={docType.id}
                    className="flex items-start justify-between gap-3 rounded-[1.25rem] border border-border/70 bg-background/80 px-4 py-3"
                  >
                    <label className="block" htmlFor={checkboxId}>
                    ...
                    <input
                      id={checkboxId}
                      type="checkbox"
                      aria-label={`Toggle ${docType.label}`}
