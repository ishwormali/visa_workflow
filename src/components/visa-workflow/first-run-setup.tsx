...
export function FirstRunSetup({
  ...
  return (
    <section className="panel p-6 sm:p-8">
      <p className="text-xs tracking-[0.24em] text-muted-foreground uppercase">Config</p>
      <h2 className="mt-3 font-heading text-3xl font-semibold">First-run setup</h2>
      <p className="mt-3 max-w-3xl text-sm leading-6 text-muted-foreground">
        Pull the latest Document list, parse the recurring document definitions, and keep only the
        ones you want automated each month. Configure Google Drive root folder from Settings.
      </p>

      <div className="mt-8 rounded-[1.75rem] border border-border/70 bg-card/80 p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs tracking-[0.24em] text-muted-foreground uppercase">Seed</p>
        ...
        {seedError ? (
          <div className="mt-4 rounded-3xl border border-destructive/20 bg-destructive/8 p-4 text-sm text-destructive">
            ...
            <pre className="mt-3 overflow-x-auto font-mono text-xs whitespace-pre-wrap text-foreground">
              {seedSource}
            </pre>
          </div>
        ) : null}
      ...
      {seedReview.length ? (
        <div className="mt-8 space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs tracking-[0.24em] text-muted-foreground uppercase">Review</p>
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <label className="inline-flex items-center gap-2 rounded-full border border-border/70 bg-background px-3 py-2 text-sm">
                    <input
                      ...
                      onChange={(event) =>
                        onToggleSeedReviewDocType(docType.id, event.target.checked)
                      }
                    />
