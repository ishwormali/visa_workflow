...
const Step1Scan = ({ docDates, onNext, onBack }) => {
  const detectedByDoc = {};
  ...
  return (
    <>
      <StepHead
        eyebrow="Step 1 · Scan"
        title={<>Reading <em>Drive</em></>}
        desc={<>Searching <span className="mono">Documents requested</span> recursively and matching files to active doc types via regex.</>}
      />
                ...
                <div className="doc-row" key={d.id}>
                      ...
                      <div className="doc-files">
                        {files.map(f => (
                          <div className="doc-file" key={f.name}>
                            ...
                            <span>{f.name}</span>
                            <span className="dim" style={{marginLeft: 'auto'}}>{f.size}</span>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div>
                    <Badge kind={files.length ? 'detected' : 'pending'}>
                      {files.length ? `${files.length} ${files.length === 1 ? 'file' : 'files'}` : 'pending'}
                    </Badge>
                  </div>
                </div>
              );
