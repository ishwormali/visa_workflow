// Step 0 — Setup, with per-doc dates

const Step0Setup = ({ docDates, setDocDates, emailConfig, onNext, onEditEmail }) => {
  ...
  const allValid = ACTIVE_DOCS.every(d => {
    ...
    if (d.category === 'gdoc_photos') return true; // dates per photo
  ...
  return (
    <>
      <StepHead
        ...
        title={<>This month's <em>submission</em></>}
      ...
      <div className="notice">
        ...
        <div>
          Last submitted on <strong>{lastSession.submittedAtPretty}</strong> with <strong>{lastSession.docs.length} documents</strong>. That's <strong>9 days ago</strong>.
        </div>
      </div>
      ...
      <div className="panel">
        <div className="panel-head">
          ...
          <button className="btn btn-ghost btn-sm" onClick={onEditEmail}>Edit ↗</button>
        </div>
                ...
                <div className="doc-row" key={d.id}>
                  ...
                  <div className="doc-main">
                    ...
                    <div className="doc-meta">
                      ...
                      <span className="doc-meta-tag">{d.dateFormat === 'range' ? 'date range' : 'single date'}</span>
                      {d.category === 'gdoc_photos' && <span className="doc-meta-tag">dates per-photo</span>}
                    </div>
                        ...
                        {d.dateFormat === 'range' ? (
                          <>
                            <div className="date-row">
                              <label>FROM</label>
                              <input className="date-input" type="date" value={v.from || ''} onChange={e => updateDate(d.id, 'from', e.target.value)} />
                              <label style={{width: 'auto', marginLeft: 8}}>TO</label>
                              <input className="date-input" type="date" value={v.to || ''} onChange={e => updateDate(d.id, 'to', e.target.value)} />
                            </div>
                          </>
                        ) : (
                          <div className="date-row">
                            <label>DATE</label>
                            <input className="date-input" type="date" value={v.single || ''} onChange={e => updateDate(d.id, 'single', e.target.value)} />
                          </div>
                        )}
                      </div>
                    )}
      ...
      <details style={{marginTop: 12}}>
        <summary style={{cursor:'pointer', fontFamily:'var(--font-mono)', fontSize: 11, color:'var(--ink-3)', padding: '8px 0'}}>
              ...
              {MOCK_DOC_TYPES.filter(d => !d.active).map(d => (
                <div className="doc-row" key={d.id} data-inactive="true">
                  ...
                  <div className="doc-main">
                    <div className="doc-label">{d.label}</div>
                    <div className="doc-meta">
                      <span className="doc-meta-tag">{d._note}</span>
                    </div>
                  </div>
                  <div><Badge kind="inactive">inactive</Badge></div>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="btn-row between">
        <button className="btn btn-ghost btn-sm">⟳ Re-seed from <span className="mono" style={{marginLeft: 4}}>Document list</span></button>
